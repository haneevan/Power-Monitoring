...
def get_weekly_summary():
    try:
        ...
        # We query the single 'readings' table but filter by unit_id
        query = """
        ...
        u1_data = cursor.fetchall()
        
        # Fetch Unit 02 data
        cursor.execute(query, ("unit02",))
        u2_data = cursor.fetchall()
        
        db.close()

        # Extract labels from Unit 01 (primary)
        labels = [row[0] for row in u1_data]
        
        # If Unit 01 is empty but Unit 02 has data, use Unit 02 labels
        if not labels and u2_data:
            labels = [row[0] for row in u2_data]

        return jsonify({
            "labels": labels,
            "u1": [round(row[1], 3) for row in u1_data],
            "u2": [round(row[1], 3) for row in u2_data]
        })
    except Exception as e:
