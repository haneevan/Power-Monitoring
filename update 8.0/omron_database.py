...
def get_historical_readings(days=1, unit_id="unit01"):
    """Fetches records for a specific unit for the last X days."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    query = """
...
def get_historical_readings_by_range(start_date, end_date, unit_id="unit01"):
    """Fetches records for a specific unit between two dates."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    query = """
        SELECT * FROM readings 
        ...
        ORDER BY timestamp ASC
    """
    cursor.execute(query, (unit_id, start_date, end_date))
    rows = cursor.fetchall()
    conn.close()
    return [Reading(*row) for row in rows]
