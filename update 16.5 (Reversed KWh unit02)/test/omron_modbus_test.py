...
REGISTER_KWH = 0x0200  # Kwh Active
REGISTER_KWH2 = 0x0202 #regenerative

# --- 3. SCALING FACTORS (INFERRED FROM IMAGE EXAMPLES) ---
SCALE_V  = 0.1 #0.01        
SCALE_A  = 0.01 #0.001       
SCALE_KW = 0.001 #0.0001
SCALE_KWH = 0.001   
SCALE_KWH2 = 0.001   
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                print(f"Power/hour (kWh):  {kwh_value:.3f} (Raw: {raw_kwh})")
                
                # 4.4. Read regenerative(kWh2)
                raw_kwh2 = instrument.read_long(REGISTER_KWH2, FUNCTION_CODE, signed=True)
                kwh2_value = raw_kwh2 * SCALE_KWH2
                print(f"Power/hour regenerative(kWh 2):  {kwh2_value:.3f} (Raw: {raw_kwh2})")
