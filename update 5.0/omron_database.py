...
def get_historical_readings_by_range(start_date_str, end_date_str):
    ...
    try:
        ...
        # --- Granularity Selection Logic (Unchanged) ---
        if total_days <= 1:
            INTERVAL_MINUTES = 5 
        elif total_days <= 7:
            INTERVAL_MINUTES = 30
        ...
        # SQLite Grouping Key 
        # Added :02d formatting to ensure two digits for minutes, preventing SQL errors.
        GROUPING_QUERY = f"""
        strftime('%Y-%m-%d %H:', timestamp) || 
        (CAST(strftime('%M', timestamp) AS INT) / {INTERVAL_MINUTES}) * {INTERVAL_MINUTES:02d} ||
        ':00'
        """
        ...
        with sqlite3.connect(datafile, timeout=5.0) as conn:
            ...
            FROM readings
            -- CRITICAL FIX: Use >= start and < (end_date + 1 day) for accurate range filtering
