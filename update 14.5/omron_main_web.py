...
def monitoring_loop(client):
    ...
    while logging_running:
        ...
        try:
            ...
            if data1:
                # FIXED: Added data1['val_power_kw'] to the arguments
                log_reading(
                    data1['val_voltage'], 
                    data1['val_current'], 
                    abs(data1.get('val_power_kw', 0.0)), # The missing kW argument
                    data1['val_energy_kwh'], 
                    "unit01"
                )
                with cache_lock:
        ...
        try:
            ...
            if data2:
                # FIXED: Added data2['val_power_kw'] to the arguments
                log_reading(
                    data2['val_voltage'], 
                    data2['val_current'], 
                    abs(data2.get('val_power_kw', 0.0)), # The missing kW argument
                    data2['val_energy_kwh'], 
                    "unit02"
                )
                with cache_lock:
    ...
    try:
        ...
        query = """
            SELECT strftime('%m/%d', timestamp) as day, 
                   AVG(val_current) as avg_val -- Use ABS() in the SQL query
            FROM readings
            WHERE unit_id = ? 
        ...
        # Create a unique, sorted list of all dates present in either unit
        all_dates = sorted(list(set(u1_dict.keys()) | set(u2_dict.keys())))

        # Map data to those dates, filling missing days with 0
        return jsonify({
            "labels": all_dates,
...
def get_weekly_energy_summary():
    try:
        ...
        # Convert ms to seconds (e.g., 1000ms -> 1s)
        # If your config isn't accessible here, replace 'interval_sec' with your actual number
        interval_sec = 1.0  
        
        # Query Explanation:
        # 1. SUM(ABS(val_power_kw)) -> Total of all power readings (ignoring negative signs)
        # 2. * interval_sec -> Total 'kW-seconds'
        # 3. / 3600 -> Convert 'kW-seconds' to 'kWh'
        query = f"""
            SELECT strftime('%m/%d', timestamp) as day, 
                   (SUM(ABS(val_power_kw)) * {interval_sec}) / 3600.0 as daily_kwh
            FROM readings
            WHERE unit_id = ? 
            AND timestamp >= date('now', '-7 days')
            GROUP BY day
