$(document).ready(function() {
    ...
    $('#historyForm').on('submit', function(e) {
        ...
        $.getJSON(`/api/${config.unitId}/history`, { start_date: start, end_date: end }, function(data) {
            ...
            // Only update history charts, leave live charts alone
    ...
    // 6. FIXED Reset to Live Mode (No Refresh)
    $('#resetLiveBtn').on('click', function() {
        currentData = config.history; // Reset currentData for CSV
        updateHistoryOnly(config.history); // Reset History Charts
