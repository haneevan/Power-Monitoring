...
from flask import Flask, render_template, request, jsonify # <-- FIX: Added request, jsonify
...
# FIX: Added get_historical_readings_by_range
from omron_database import setup_database, log_reading, cleanup_old_data, get_latest_readings, get_historical_readings, get_historical_readings_by_range 
...
# API ROUTE for date range selection
@app.route('/api/history', methods=['GET'])
def api_history_range():
    """
    API endpoint to return historical readings within a specified date range.
    Expects 'start_date' and 'end_date' query parameters (YYYY-MM-DD).
    """
    start_date = request.args.get('start_date')
    end_date = request.args.get('end_date')

    if not start_date or not end_date:
        return {'error': 'Missing start_date or end_date parameter'}, 400

    try:
        # Fetch data using the intelligent database function
        history_data_tuples = get_historical_readings_by_range(start_date, end_date)
        
        # Convert list of namedtuples to list of dictionaries for JSON
        history_data_dicts = [item._asdict() for item in history_data_tuples]
        
        return history_data_dicts, 200
        
    except Exception as e:
        print(f"Error fetching history data: {e}")
        return {'error': 'Internal server error during data retrieval'}, 500
