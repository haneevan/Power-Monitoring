...
def get_weekly_energy_summary():
    try:
        ...
        # New Query Logic: 
        # For each day, we take the highest reading and subtract the lowest reading
        # This represents the total kWh 'traveled' by the meter that day.
        query = """
            SELECT strftime('%m/%d', timestamp) as day, 
                   (MAX(val_energy_kwh) - MIN(val_energy_kwh)) as daily_kwh
            FROM readings
            WHERE unit_id = ? 
            AND timestamp >= date('now', '-7 days')
            GROUP BY day
