        ...
        try:
            ...
            if data1:
                ...
                log_reading(
                    data1['val_voltage'], 
                    data1['val_current'], 
                    data1.get('val_power_kw', 0.0), # The missing kW argument
        ...
        try:
            ...
            if data2:
                ...
                log_reading(
                    data2['val_voltage'], 
                    data2['val_current'], 
                    data2.get('val_power_kw', 0.0), # The missing kW argument
...
def get_weekly_energy_summary():
    """Calculates daily energy consumption (Max - Min kWh) for the last 7 days."""
    try:
        db = sqlite3.connect('omron.db')
        cursor = db.cursor()
        
        # We calculate the delta (MAX - MIN) for each day
        query = """
            SELECT strftime('%m/%d', timestamp) as day, 
        ...
        # Create a master list of dates so U1 and U2 bars match perfectly
        all_dates = sorted(list(set(u1_dict.keys()) | set(u2_dict.keys())))
